---
type: mixed
density: rich
style: notion
palette: default
image_count: 6
---

## Illustration 1

**Position**: TL;DR / 首个说明段落之后

**Purpose**: 沿时间轴说明状态如何变化，以及何时需要人工介入。

**Visual Content**: 将本节真实术语映射为无文字的 Notion 知识卡片、图标和连接关系。

**Type Application**: timeline

**Filename**: 01-timeline-lifecycle-timeline.svg

## Illustration 2

**Position**: 先分清四个盒子 / 首个说明段落之后

**Purpose**: 把本节的关键概念拆成一张模块化知识地图。

**Visual Content**: 将本节真实术语映射为无文字的 Notion 知识卡片、图标和连接关系。

**Type Application**: infographic

**Filename**: 02-infographic-concept-map.svg

## Illustration 3

**Position**: Thread History 怎样恢复任务 / 首个说明段落之后

**Purpose**: 把动作顺序和反馈关系转换成端到端流程。

**Visual Content**: 将本节真实术语映射为无文字的 Notion 知识卡片、图标和连接关系。

**Type Application**: flowchart

**Filename**: 03-flowchart-operating-flow.svg

## Illustration 4

**Position**: Memory 是召回层，不是规则层 / 首个说明段落之后

**Purpose**: 并排呈现容易混淆的选项、边界与取舍。

**Visual Content**: 将本节真实术语映射为无文字的 Notion 知识卡片、图标和连接关系。

**Type Application**: comparison

**Filename**: 04-comparison-boundary-comparison.svg

## Illustration 5

**Position**: /import 究竟从 Claude Code 和 Cursor 搬了什么 / 首个说明段落之后

**Purpose**: 用门禁与检查点总结验证证据和安全护栏。

**Visual Content**: 将本节真实术语映射为无文字的 Notion 知识卡片、图标和连接关系。

**Type Application**: infographic (guardrail)

**Filename**: 05-infographic-verification-guardrails.svg

## Illustration 6

**Position**: Cursor 有原生导入，仍要做语义验收 / 首个说明段落之后

**Purpose**: 把组件、角色或层级关系画成可扫描的框架图。

**Visual Content**: 将本节真实术语映射为无文字的 Notion 知识卡片、图标和连接关系。

**Type Application**: framework

**Filename**: 06-framework-system-framework.svg
