...
本次实测(L1,统一后端但协议偏差——Letta 是 agent 直接答):冒烟 12/12 = 100%,全场最高。glm-4.7 的 tool-coding 足够可靠,12 题全对(FF 号、饮食、2 猫、工作、预算、公寓、护照、会议日期、偏好、前职、姓名、Pepper 年龄)。LongMemEval 未跑通——agent 逐消息 tool call 在 9K+ token 多会话 haystack 上太慢,首题只摄入 1 个 session 就 600s 超时。
...
- **冒烟 100%(12/12)**:全场最高。agent 自管记忆 + glm-4.7 强 tool-calling,事实回忆极佳。
...
这些边界说明 Letta 最适合"中等规模持续对话 + 强模型 + 需要 agent runtime 不只是记忆"的场景。在这个场景下它的事实回忆能力是本次评测全场最强的(100%)。
