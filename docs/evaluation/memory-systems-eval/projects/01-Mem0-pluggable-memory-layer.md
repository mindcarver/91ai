...
独立的第三方报告差异巨大:Atlan 用 GPT-4o 复现仅 49.0%(对比 Zep 63.8%);Zep 还发过一篇 rebuttal 说 Mem0 的 SOTA 被夸大。本次评测在统一 qwen2.5:14b 后端下实测:冒烟 83.3%(10 正确 / 1 部分 / 1 错),LongMemEval 代表性子集 1/3 ≈ 33%(temporal 题因 dedup 卡 20 分钟跳过)。这个 L1 数字既不是 94% 也不是 49%——它在统一后端、固定 reader / judge 下是 83% / 33%,这才是和别的工具可比的数。
...
建议标签:**先试点**(冒烟 83% 但 LongMemEval 仅 33%,且自报 94% 含水分)。采用前务必在你自己的真实多会话数据上、统一后端下重跑,而不是信 README 的 93.4%。
