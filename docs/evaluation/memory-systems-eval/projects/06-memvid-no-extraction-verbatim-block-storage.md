...
Memvid 是一条"反主流"的记忆 / RAG 路线:它不做任何 LLM 事实抽取,把文本 / 文档切成块,每块编码进一个视频文件(v1 用 QR 码帧编进 MP4;v2 是 Rust 单文件 B-tree/HNSW),检索时按 BM25 + 向量相似度返回原始文本块。它的卖点是零依赖(单文件)、零 LLM(纯 chunk + embed)、能扛超大规模、值零丢失。它的局限是:没有聚合、没有时序、没有知识更新能力——它是 RAG 替代,不是真正意义上的"用户画像 / agent 记忆"。
...
本次实测(L1):**冒烟 83.3%**(10/1/1,与 Mem0 并列);**LongMemEval 代表性子集 25%(1/4)**——single-session 对、multi-session 错(不会计数)、temporal 错(召回漏)、**knowledge-update 错(返回旧值 27:12 而非更新后的 25:50)**;**完整 138K token haystack single-session 对**(从 9MB 视频里正确召回 degree)。
...
- **冒烟 83.3%**:无抽取,值零丢失,与 Mem0(事实抽取)并列。强对比:小规模上"无抽取逐字块"追平"最佳事实抽取"。
- **LongMemEval 25%**:暴露逐字块的结构性弱点——multi-session 聚合败、temporal 败、knowledge-update 败(返回旧值)。
- **完整 138K haystack 可行**:9MB 视频、549 QR 帧、single-session 正确召回。证明它能扛真实规模(瓶颈不在吞吐,在结构能力)。
...
它的真正价值场景:你有一个大知识库(文档、历史对话、代码),想编码成一个可移植的单文件,在离线 / 边缘场景做 RAG 检索。这个场景下 Memvid 的"零依赖、单文件、扛规模、值不丢"是实打实的优势(本次 138K 实测验证了扛规模)。但如果你要的是"让 agent 记住用户的长期画像和偏好演变",选 Mem0 / Letta / Supermemory,不要选 Memvid。
...
所以 85.65% 是云产品的数,25% 是本地免费引擎(统一后端)的数。两者不可比,但都"真实"——只是测的东西不同。决策时用哪个?取决于你用云产品还是本地引擎。如果用本地(本次评测的立场),25% 是可参考的;如果上云,厂商的 85.65% 才相关(但那要付费 + 数据出境)。
...
**场景一:大规模文档 RAG(强项)。** 你有一个大文档库(产品手册、法规、代码库),要编码成可移植单文件做"找相关片段"。Memvid 在这里强——零依赖、单文件、扛规模(138K 实测)、值不丢。预期表现:高(找到相关片段)。
