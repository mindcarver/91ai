...
本文面向已经使用 Claude Code、MCP 和 Webhook，准备把外部系统接入编码会话的中级开发者。重点不是聊天机器人配置，而是事件从哪里进入、何时会丢、权限在哪一层收口。
...
本稿基线环境的 `claude --version` 为 `2.1.185`，其帮助文本没有显示 `--channels`。因此本文不把后续流程描述成已在该环境亲测成功。官方文档仍把以下命令列为 Research Preview 用法，实际启用还受账号类型、组织策略和 CLI 分发状态约束。
...
外部消息是非可信输入，不应因为它来自团队聊天就自动获得命令权限。至少要分开处理三类控制：
