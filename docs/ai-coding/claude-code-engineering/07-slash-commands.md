...
Slash Command 的本质：**把散落在每个人脑子里的任务模板，变成仓库里的版本化文件。** 它和 CLAUDE.md 的区别是加载时机——CLAUDE.md 每次会话全量加载，Command 只在调用时加载。这意味着你可以大胆写长提示词，不用担心拖慢日常会话。

这带来一个关键推论：**命令的长度不会惩罚不使用它的人。** 一个 80 行的 `/review` 命令，只有调 `/review` 的那个会话才承担这 80 行的 Token 成本。其余 99% 的会话完全不受影响。这个属性让命令成为"安全的长提示词容器"——你可以把审查标准写得极其详细，不用担心像 CLAUDE.md 那样挤占全局上下文。
