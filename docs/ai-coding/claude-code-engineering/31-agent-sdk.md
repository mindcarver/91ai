...
SDK 不改变 Claude Code 的核心能力，而是把它从"终端里的交互式工具"变成"你代码中的一个可编程组件"。你可以控制它的启动参数、运行时权限、工具集合、会话生命周期和输出格式。
...
SDK 集成中，subagent 模式（见第 12 篇）特别有价值。你可以编排多个专业化的 Agent 实例协同工作，每个实例有独立的工具权限和上下文窗口。
