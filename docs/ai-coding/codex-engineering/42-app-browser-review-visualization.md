...
资料基线：2026-07-22。功能状态以 OpenAI 官方文档与 changelog 为准。Sites 是 public beta，Visualizations 在桌面端仍处于滚动发布的 preview。本文没有执行真实网站发布、登录账号操作或 PR 写入，因此涉及这些动作的描述均限定为官方文档边界。
...
Visualizations 能生成图表、地图、图解、计算器和模拟器。桌面端目前处于 preview 滚动发布，可用性取决于计划、平台、账号和工作区。Codex CLI 与 IDE 扩展不渲染这类结果。
