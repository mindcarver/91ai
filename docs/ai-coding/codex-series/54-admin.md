...
| `network` | 配置 | 网络访问约束 |
| `managedAllowedDomainsOnly` | 布尔值 | 是否只允许已管理的域名 |
...
allowed_approval_policies = ["suggest", "auto-edit"]
allowed_permissions = ["basic"]
```

设置了这些策略后，用户尝试使用 `danger-full-access` sandbox 或 `full-auto` 审批策略时，Codex 会拒绝执行。
...
network.managed_allowed_domains_only = true
```

`enforce_residency` 确保数据不离开指定区域。`allowed_web_search_modes = ["off"]` 禁止 Codex 访问 web 搜索。`managed_allowed_domains_only` 限制 Codex 只能访问管理员批准的域名。
