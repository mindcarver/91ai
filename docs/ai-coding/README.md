...
| 长文 | 角度 |
| --- | --- |
| [上生产：从单次会话到可信赖的代理控制系统](./loop-engineering/long-form/01-production-readiness.md) | 生产落地深度解析，统一判断框架 + readiness 自检 |
| [用 Claude Code 从零搭一条能跑的 Loop：Daily Triage 落地手册](./loop-engineering/long-form/02-build-first-loop.md) | 实操教程，含 `loop-init` 脚手架 + 完整文件内容 |
| [概念全景：把循环放进 AI 编程演进坐标的认知地图](./loop-engineering/long-form/03-concept-panorama.md) | 认知地图，prompt→context→harness→loop + 四代循环谱系 |
