...
"session-local"是关键词。一个提醒的投递，是这个会话的根 agent **等到完全空闲后**，在那个对话里排队一个普通的 follow-up turn。它**绝不** steering 当前正在做的工作，**也不加**单独的回执或提醒卡片。README 里有一句钉死的话：**Schedule 不提供浏览器、操作系统、邮件、短信或其他外部通知。**
...
`every_seconds` 的 300 秒下限值得注意。这是个防滥用闸：不允许高频轮询。如果你想要每秒触发的东西，schedule 不是工具，那是定时任务或事件循环的活。300 秒下限把 schedule 锁定在"人尺度的提醒"这个用途上。

`schedule_list` 列出当前的提醒，`schedule_delete` 删除。创建和实际删除**只在会话持久化确认了事件前缀之后**才确认成功。这保证了"我说删了"和"日志里真删了"是一致的，不会出现"工具返回成功但日志里还在"的撕裂。
...
但这条假设**只属于自然语言解释**。落到 `schedule_create.at` 这个机器字段上，时区必须明确：
...
**原始的会话日志拥有每个提醒。** 一个 live 的根 agent 等到完全空闲，然后排队一个 follow-up turn。但那个"等到时间到"的定时器是**进程内**的内存状态。

所以：

- **关闭进程或让会话变冷**，停止内存里的定时器，**但不删记录**。
- **重新打开同一个会话**，恢复等待，**补投过期的提醒**。
- **读冷历史**（只是看日志）**永远不激活**它。只有真正重开会话、让 agent 活过来，才会恢复等待。
- **fork 不继承**父的提醒。
...
- 只有**最新该到的那次**被呈现。对于一个固定频率的提醒，下次目标仍按原始序列算，不因为补投而漂移。
- 在同一个空闲决策点上，所有不同的、同时过期的 Every 记录，**合并成一个 follow-up**，每个出现一次。**错过的间隔不积压成 backlog。**
- 到期的一次性提醒在该批次**之前**跑。

"错过的间隔不积压"这条很重要。假设有个每 5 分钟的提醒，进程关了一小时。朴素做法会补投 12 次（一小时除以 5 分钟），把对话刷爆。这里只补一次（最新那次），下次仍按原序列。这把"长时间离线后的补投"控制在可接受范围内。
...
3. 根 agent **等到完全空闲**。
4. 在那个对话里，排队一个**普通的 follow-up turn**。没有单独的提醒卡片，没有特殊 UI，就是一条普通消息。
5. agent 处理这条消息，就像处理任何用户消息一样。

一条持久派发只记录"follow-up 已排队"，**不确认**模型成功或用户收到。因为一旦排队进 inbox，这就是一次普通的 turn，能不能做好取决于 agent，不取决于 schedule。

这条链路复用了 agent 的 inbox FIFO 机制（子 agent 那篇讲过，inbox 是唯一队列）。提醒不是特殊的投递通道，它就是一条进 inbox 的消息。这让 schedule 不用造一套独立的消息系统，直接搭 session 的便车。

## 它怎么挂进来：overlay，不是核心

回顾整个系列反复出现的主题：一切皆插件，ship 为 overlay。`web-schedule` 是这个主题的一个具体例子。
...
README 列的限制很清楚，值得逐条记住：

- **不支持 Calendar 和 Cron 表达式。** 只有 `after_seconds`、`at`、`every_seconds` 三种。想要"每周一早上九点"这种复杂调度，schedule 做不了，得用别的工具。
- **`every_seconds` 最少 300 秒。** 防高频滥用。
- **没有外部通知。** 不发浏览器、系统、邮件、短信通知。提醒只是会话内的 follow-up turn。
- **必须进程开着、agent 空闲才投递。** 进程关了不投递，重开补投。
- **fork 不继承提醒。** 子会话不带走父的提醒。
- **只确认"已排队"，不确认"模型成功或用户收到"。**
- **`at` 必须带明确时区。** 没有 session 默认时区。

## 权衡与局限

**它不是通用定时任务系统。** 没有 cron、没有外部通知、有最小间隔。它只解决"在 agent 会话内安排一个延迟的后续动作"这个窄场景。拿它当 crontab 用会失望。

**依赖进程开着。** 想确保提醒按时投递，得让 `dsh web` 进程一直开着、那个会话的 agent 空闲。这对"长期挂着的开发助手"场景合理，对"偶尔打开用一下"的场景不可靠（虽然重开会补投，但补投是有延迟的）。

**补投可能滞后且合并。** 长时间离线后，过期的提醒合并成一次补投，下次按原序列。这意味着你不会看到每个错过的间隔，只看到最新那次。对"统计错过了几次"的需求不友好。

**时区严格增加调用复杂度。** 每次创建都得带明确时区，模型不能依赖隐式默认。这防了 bug，但模型或调用方得记得带时区，忘了会被拒。

**overlay 不是核心。** 它是 example，不在默认组合里。一个不知道这个 overlay 的部署，模型根本没有 schedule 工具。要用得显式 patch 进去。

## 结论

`web-schedule` 是一层 session 内的自动化 overlay，通过 `--patch` 挂进 `dsh web`。它提供 `schedule_create`/`schedule_list`/`schedule_delete` 三个工具，支持 `after_seconds`、`at`、`every_seconds`（最少 300 秒）三种触发方式。

它的本质是"被推迟的 follow-up turn"：提醒记在会话日志里，等根 agent 完全空闲时，在那个对话里排队一条普通消息。没有外部通知，没有 cron，没有 calendar。时间走绝对权威：浏览器 IANA 时区只管自然语言解释，`at` 必须带明确时区，没有 session 默认时区，夏令时 gap 拒绝、overlap 选第一个，只存 UTC 目标。

冷热恢复的边界清晰：定时器是进程内的、记录是持久的。进程关了定时器停，重开会话补投过期提醒，读冷历史不激活，fork 不继承。补投合并、不积压，下次按原序列。

几个判断：它不是通知系统，是会话内自动化；时间绝对权威、无 session 默认时区；定时器与记录分离让崩溃恢复可靠；提醒复用 inbox follow-up 机制，不造独立消息系统；作为 overlay 体现"一切皆插件、按需引入"。这个例子虽小，却把整个系列的核心主题（插件化、session 日志为真相、复用既有机制、fail loud、显式优于隐式）在一个具体功能上集中展示了一遍。
