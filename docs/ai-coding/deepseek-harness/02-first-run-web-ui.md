...
> 看完你应该能独立完成一次完整的 Web UI 会话，并对"一个 agent harness 在第一次运行时到底做了哪些事"有体感——这体感是后面 47 篇架构拆解的地基。
...
第一次跑完，你大概会对 `dsh` 形成一个直觉：它不是"聊天框 + 代码补全"，而是一个会主动读文件、跑命令、在你授权下改东西的代理。这个直觉是对的，但还很粗。本系列剩下的 47 篇，就是在把这个直觉拆成可理解的机制。
...
一个建议：**保留你第一次跑的那个会话**。`dsh` 的会话是可 fork、可 resume 的（09、10 篇），第一次的会话日志是个现成的样本，后面读源码导读时可以拿它当例子对照。
