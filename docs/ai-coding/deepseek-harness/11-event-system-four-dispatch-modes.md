...
> 如果这一篇你只能带走一句话，带走这句：Cordis 的事件有五种派发模式，每种回答一个不同的问题（广播？并发？顺序拍板？同步拍板？层层包裹改写？），其中 waterfall 是 around 中间件，调 `next()` 才往下传、不调就是短路否决，这套语义是 DeepSeek Harness 实现"拦截与策略"的全部地基。
