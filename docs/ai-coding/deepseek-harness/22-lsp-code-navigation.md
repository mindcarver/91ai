...
> 如果只能从这篇带走一句话：`dsh` 把 LSP 做成一个只暴露四个归一化操作、且**没有任何协议逃生舱**的能力接缝，模型永远只问"跳定义、查引用、跳实现、hover"这四件事，原始 JSON-RPC、文档同步、进程控制全部藏在 provider 后面，换语言服务器不影响模型怎么问。
...
几个判断值得带走：四个操作是故意选的最小集，符号和调用层级因为需要不同 schema 暂时不做；无逃生舱是主动取舍，用有限能力换跨 provider 的模型工具稳定；坐标用零基 UTF-16 对齐协议，工作区相对化必须用 provider 给的 `resolvedWorkspaceUri`；findReferences 自带声明，错误按稳定码路由。
