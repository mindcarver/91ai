...
- `FEEDBACK_ONLY`：只在用户主动记录反馈（`feedback/record` 事件）时，把规范会话日志截至那条事件的部分回放、投影、脱敏后交给后端；更晚的记录等下一次反馈，没有就留在本地。适合"只在用户主动提交问题时才共享会话数据"的场景。
- `DISABLED`（默认）：不构造 coordinator、provider、processor、exporter，没有记录离开进程。一个 `feedback/record` 会记下"telemetry 已禁用，反馈保留在本地"。这是安全默认值。
...
`dsh` 的解法是固定 chunk 投影：每个 `(turn, step)` 只发第一个 `assistant/chunk`，其余在捕获时丢弃。
...
telemetry 的交付是尽力而为（best-effort）。协调器维护一个 `WeakMap<Session, seq>` 标记每个会话已移交的最高 seq。注意：是"已移交"，不是"已交付"。
...
- ops 通道：操作信号。只有两个信号：`agent-error`（agent 级错误）和 `shutdown`（会话或进程终止）。它们故意省略 `event.seq` 和 `event.type`，永远不会被误认为 ledger 行。ops 记录是用来告警的信号，不是用来求和的条目，它们容忍重复。
...
但其他敏感信息（文件内容里的密码、命令输出里的 token）会在记录里，除非部署挂载了脱敏规则。`dsh` 不替你做这个决定。它提供了 `session-telemetry/record` 瀑布作为扩展点，但规则由部署自己写。

## 共享披露

mounted backend 通过必选的 `sharing` 成员向人类可见的确认界面（`/feedback` 命令）披露当前策略：`full` 是每个事件实时移交，`feedback-only` 是只在反馈触发时回放，`disabled` 是不移交任何东西。
...
代价有三份。交付是尽力而为：没有 durable outbox，崩溃时队列里的记录丢，resume 不回填，需要 crash-recovery 的部署要等 outbox 特性落地。脱敏零内置：没有规则时，原始捕获副本离开进程，文件内容和命令输出里的凭证也一起走，脱敏规则因部署而异，`dsh` 不替你猜，部署自己负责。`FEEDBACK_ONLY` 不保留 telemetry 快照：反馈触发时才用当前挂载的规则读当前日志，反馈前崩溃什么都不上传，反馈前改策略会影响那次回放导出的内容。

回报对应也有三份。捕获不拖累 agent loop，`emit()` 非阻塞、异常被 contain；上报栈完全可替换，实现 `SessionTelemetrySink` 就能换通道；模式让"什么离开进程"成为显式选择，共享披露让用户的确认文本诚实。上游 `@opentelemetry/sdk-logs` 还在实验树发布，SDK API 变动只影响 provider 这一层，不伤接缝契约。

## 结论

整条设计压回一条边界：harness 负责捕获、投影、脱敏扩展点，到 `emit()` 为止；批处理、重试、丢包归上报 SDK。chunk 投影控制体积，ledger/ops 两通道区分"可求和的条目"和"可告警的信号"，交付尽力而为、接收方按 `(session.id, event.seq)` 去重。换来的是一条不拖累 loop、不预设上报栈、把共享决定留给部署和用户的可观测性通道。
