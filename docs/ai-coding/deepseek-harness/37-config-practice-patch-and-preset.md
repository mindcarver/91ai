...
> `dsh` 的配置改法是"按 id 替换整行，不做深度合并"，这意味着改一个子系统的行为，在 patch 层写一行同 id 的配置就能覆盖它，不用 fork 源码。
> 这一篇拆三件事：插件 Config 怎么定义、patch 怎么按 id 替换、preset 怎么把一组插件打包成分发单元。

## 配置的心智模型：cordis.yml 是一棵树

`dsh` 启动时，从 profile 指定的 bundle 序列里加载 patch 层，叠出一棵插件树。这棵树是一个扁平的 YAML 数组，每行是一个插件条目：
...
插件作者在插件入口里导出三样东西：一个 `name` 常量、一个 `Config` 接口、一个同名的 Schemastery schema。schema 用 `Schema.object` 逐字段声明类型和默认值，比如 `Schema.string().default('Hello')`；`apply(ctx, config)` 拿到的，就是校验和填完默认值之后的配置。

schema 做两件事：校验和填默认值。Cordis 加载插件时用 schema 校验 cordis.yml 里的 config，校验不过就在加载时报错。默认值直接写在 schema 字段上，不单独维护。

`dsh` 有一个硬性的设计原则：**任何两个部署可能想设置不同的值，都必须是配置字段。** 测试方法是：cordis.yml 能不改代码就改变这个值吗？如果不能，它就被硬编码了，这是错的。错的做法是把超时写成模块常量 `const TIMEOUT = 30000`；对的做法是在 `Config` 接口里加 `timeoutMs` 字段，默认值 30000 由 schema 填。
...
这是 `dsh` 配置系统最关键的一条规则：**一个 patch 替换目标行的整个 config，不做深度合并。**

假设 base bundle 注册了一行 id 为 `webserver` 的条目，config 里设了 `host` 和 `port`。你在自己的 patch 里写一行同 id 的条目，config 里只设 `port: 3081`。结果是 webserver 的 config 变成只有 `port: 3081`，`host` 被丢了，因为你替换了整个 config 对象。

这个设计看起来不方便，实际上很安全。深度合并的问题在于：你不清楚一个字段是"你没有设置所以继承默认值"还是"你显式设置成了这个值"。replace 语义让每次覆盖都是明确的、完整的，没有隐式继承的歧义。

只想改一个字段而保留其余的，有两条路：要么靠 schema 默认值兜住其余字段（这是推荐做法），要么在 patch 里写完整的 config。

插入新行用 `insert`：patch 里写一个 `insert` 键，值是条目列表，格式和顶层条目一样，它们被追加到插件树的末尾。
...
举个具体的例子：把 web profile 的端口从 3080 改到 9090，并在 web 上挂一个 MCP 记忆服务器。创建 `$DSH_HOME/profiles/web/cordis.patch.yml`：
...
保存后，profile patch 是 watched 的，下一次启动或 HMR 触发时就生效。webserver 的 config 被完整替换（包括 host 和 port），memorix 作为新行被插入。
...
一个细节：如果一行 config 的某个字段没出现在 dump 里，可能是它用了 schema 默认值。dump 显示 cordis.yml 里显式设置的值和 patch 覆盖的值，schema 默认值在运行时填入但不一定显示。

排查语境的完整用法（插件没生效、config 值不对、加载顺序问题的排查路径）见 [38 篇](./38-debugging-and-troubleshooting.md)。

## Preset 系统：可分发的 agent 组合

profile 和 bundle 解决的是"怎么叠出你的插件树"。preset 解决另一个问题：怎么定义一组 agent 的配置，让它们可以被选择和切换。

`@deepseek-ai/dsh-agent-presets` 插件管理 preset 目录。它的 Config 有三个字段：`default` 是没指定时挂载的 preset id；`roots` 是扫描根目录列表，按优先级排序；`includeUserRoot` 决定是否追加 harness home 的用户 preset 目录（`<dshHome>/.agent-presets`）。

一个 preset 是一个目录，里面有一个组合定义（`agent.cordis.yml`）。preset 有两个信任级别：`system` 随部署发布，可信；`user` 本地创建（人写的或 agent 写的），信任级别等同 shell 访问。

preset 的行从 host 组合里解析，不从 preset 目录里解析：裸包名按 host 组合的基准去找（preset 目录通常在用户主目录下，Node 的向上查找够不到 harness 的依赖），相对路径仍从 preset 自己的目录解析。所以 preset 不是一棵独立的插件树，是挂在 host 组合上的参数化选择。
...
怎么知道每个插件接受哪些配置字段？看 `docs/config-catalog.md`。这个文件由 `scripts/gen-config-catalog.ts` 从源码自动生成，包含每个可加载包的完整 config 声明（JSDoc 包含在内）。它不是手写的，是编译时事实。

生成器还做交叉校验：把 runtime Schemastery schema 和粘贴的 config 声明做交叉检查，确保每个 schema 校验的 key（包括嵌套 key）都能在声明的 config 类型上定位到。这样粘贴就不能藏一个 loader 接受但声明里没有的字段。
...
`dsh` 的配置改法可以压成三句话：行为差异做成配置字段，默认值交给 schema；patch 按 id 替换整行 config，不做深度合并；preset 在 host 组合之上做可分发的参数化选择。改完跑 `--dump-config` 看层叠结果，保存 profile patch 靠 HMR 生效，不用重启进程。
