...
> 如果你只能从这篇带走一句话，带走这句：`dsh` 的配置改法是"按 id 替换整行，不做深度合并"，这意味着改一个子系统的行为只需要在 patch 层写一行同 id 的配置覆盖它，不用 fork 源码。
...
插件作者定义配置的方式是导出一个 `Config` 类型和同名的 Schemastery schema：
...
结果是 webserver 的 config 变成 `{ port: 3081 }`，不是 `{ host: '127.0.0.1', port: 3081 }`。`host` 被丢了，因为你替换了整个 config 对象。
...
`insert` 列表里的条目和顶层条目格式一样。它们被追加到插件树的末尾。
...
`--patch` 是命令行参数，用于一次性覆盖：

```sh
dsh web --patch "$PWD/examples/mcp-memory/memorix.cordis.yml"
```

这是临时加一层 patch 的方式。想持久化，把内容合并到 profile 或 home 的 `cordis.patch.yml` 里。
...
`@deepseek-ai/dsh-agent-presets` 插件管理 preset 目录。它的配置：

```typescript
export interface Config {
  default: string          // 没指定时挂载的 preset id
  roots: PresetRoot[]      // 扫描根目录，按优先级排序
  includeUserRoot: boolean // 是否追加 harness home 的 USER_PRESET_DIR
}
```
