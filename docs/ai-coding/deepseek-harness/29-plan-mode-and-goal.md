...
- 引导模型先规划再动手：在改代码前先产出一份计划给人审。这是一种协作姿态，不是硬约束。模型可以不听，但它被引导着先想后做。
- 追踪一个长期目标的进展：这个目标是什么、现在进行到哪一步（活跃、暂停、卡住、完成）、已经跑了几轮、还能跑几轮。这是持久状态，要跨重启恢复、要防并发改坏。
...
这是 plan mode 最精巧的部分。因为每个会话事件都被 turn 包住，一个用户选择会保持 pending，直到下一个被接受的 in-turn pre-step 在请求派生之前追加它。一个选择永远不强制 continuation：turn 最后一个被接受的 pre-step 之后做的选择，会在更晚的某个 turn 里追加。
...
追加失败不能阻塞 turn，选择保持 pending 等下一个。一个已追加的用户选择还记一条 plugin 来源的 `user/message` 通知，但只在上次记录的请求头描述的是另一个状态时才记，这样模型只在上下文真变了时被通知一次，绝不冗余。
...
`GoalId` 是 branded id。调用方通过 `GoalRef`（id 加正数 revision）修改一个确切的修订；每个被接受的持久变更让 revision 加一。这是比较并交换（compare-and-set）语义：你要改目标，得带上你以为是当前的 revision，防并发改坏。

持久阶段回答"目标怎么了"。`GoalPhase` 是四值联合：`active`、`paused`、`blocked`、`complete`。`blocked` 是唯一一个"因为一个问题停了"的持久状态，它的策略拥有的 reason 带一个稳定的小写 kebab-case code（用于路由）和一个自由文本 explanation（给人也给模型看）。
...
Plan mode 的软是刻意的选择，代价也实在。它没有强制力，模型在 plan mode 里直接动手改文件，拦得住的是沙箱和审批，不是它；引导和强制分开之后，引导就只是建议。pending 选择也为此付代价：追加要等 turn 边界的合适时机而不是立即落盘，进程在下一个被接受的 in-turn pre-step 之前退出，选择就丢了。

goal 的重同样有成本。每次修改都要带 revision，stale revision 被拒，调用方得处理拒绝和重试；长目标的事件持续累积，fold 成本随日志增长，靠持久化投影缓解冷读。换来的是并发修改安全、跨重启恢复，以及"进程恢复不会自动狂跑"的可控续跑。两个机制都不在 agent-loop 主干里，最小化的 headless 部署可以两者都不装，loop 照常跑，这是"一切皆插件"在目标管理上的落点。

## 结论

`ctx.planMode` 是软引导：激活时往每个请求塞一段部署拥有的 `plan:policy` 提示，状态是 `plan/mode` 事件的纯 fold，选择在 turn 边界追加，`exit_plan_mode` 要求完整计划经人审，沙箱和审批独立强制。`ctx.goals` 是持久的事件溯源目标生命周期：compare-and-set revision、四个阶段、轮次预算、持久阶段与进程内 activation 分两层，全部从 `goal/change` 事件 fold 出来。两者一轻一重，共享"状态记在日志、纯 fold 恢复、与硬强制分开"的根基，目标管理因此拆成两个重量合适的正交机制，而不是一个拧巴的大而全系统。
