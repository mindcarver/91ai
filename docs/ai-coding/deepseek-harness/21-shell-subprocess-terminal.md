...
> 如果只能从这篇带走一句话：`dsh` 把"执行命令"做成三层叠加的抽象，`ctx.subprocess` 是不带语义的底层坐标（只给原始进程事实），`ctx.shell` 在上面加出 bash 执行语义（超时、中止分类、沙箱、批输出），`ctx.terminals` 再往上加出持久交互式 PTY（就绪推断、滚动缓冲、独占发送、跨重载存活）。
