...
很多 agent 客户端的做法是：内置几个常用 MCP 服务器，或者提供一个配置面板让你填。`dsh` 的做法不同。它写了一个通用的桥接插件 `@deepseek-ai/dsh-mcp-client`，这个插件本身不实现任何具体功能，它的职责是连接一个外部 MCP 服务器，发现那个服务器的工具，然后把它们注册到 `dsh` 自己的工具注册表 `ctx.tools` 上。
...
桥接插件发现 MCP 服务器的工具后，给每个工具生成一个模型可见的公开名称：
...
如果原始名称包含非法字符或者拼接后超过 64 字符怎么办？插件不是简单截断，而是在名称后面追加一个 12 位的十六进制哈希，取自 `SHA-256(serverName + rawName)`。这个哈希保证不同的工具永远不会因为名称归一化而塌缩成同一个名字。换句话说，**工具的公开名称是 `(serverName, rawName)` 的确定性纯函数**，连接顺序、重新同步、其他服务器的存在都不会改变它。

这套机制落在源码的 `publicToolName()` 里：干净情况（没有字符替换、没超长）直接返回拼接结果，只有发生截断或字符替换时才截短名称、追加哈希。大部分工具走第一条路径。

## 注册机制：两阶段代际切换

工具注册不是"发现一个注册一个"那么简单。`dsh` 用了两阶段代际切换来保证安全：

- 阶段一（fetch）：调用 MCP 的 `tools/list`（支持分页，用 cursor 翻完），构建下一代的完整工具定义集合。这个阶段完全不碰注册表。如果发现过程中出错（网络断了、服务器列了重名工具），直接中止，上一代的注册原封不动。
- 阶段二（swap）：先销毁上一代的全部注册，再注册新的一代。如果注册时发现冲突（有别的注册占了这个命名空间），把这次的部分注册全部回滚，这个服务器的工具显示为零。

这个设计的意思是：**模型要么看到一个服务器全部工具的完整集合，要么一个都看不到，永远不会看到半套。** 源码里 `syncTools()` 用 `ToolDisposers` 管理每代的销毁器，swap 时先逐个调用旧代的销毁器，再逐个注册新工具。
...
- 连接断开时，用指数退避重连（默认初始延迟 500ms，每次翻倍，上限 30000ms）。触发点是传输关闭：stdio 子进程崩溃会触发它；HTTP 的失败按请求暴露，走 SDK 传输自己的恢复，不会被监督器重启。
- 重连成功后重新跑发现流程，恢复的工具集合替换旧集合，工具不会重复也不会泄漏。断线期间上一次成功的注册保持挂载，调用会失败直到恢复。
- 每次断线有重连预算：默认最多连续失败 10 次，超过就放弃，注销这个服务器的全部工具，直到 HMR 重载或 Host 重启。
...
MCP 服务器返回的数据来自网络（子进程也是另一个信任域），`dsh` 在解析时做了防御。MCP 的内容块（content block）有好几种类型：text、image、audio、resource。桥接插件的处理按类型分开：

- 文本类的块用换行拼接成一段文本，保留在模型上下文里，块的原始顺序不变。
- resource 链接保留名字和 URI 作为文本。
- 受支持的图片（PNG、JPEG、WebP、GIF）有条件地进入原生上下文：`ctx.attachments` 挂载、且当前调用的模型路由显式声明支持图片输入时，整批图片先解码验证再持久化为图片块。
- 被拒的图片批次、audio、内嵌 resource、不认识的块，变成明确的诊断文本，而不是消失。

完整的 JSON 块和结构化内容（`structuredContent`）保留在执行局部的规范值里，给 Code Mode 等编程化调用方用；服务器声明了受支持的 `outputSchema` 时还会校验它。二进制负载不进会话事件。

MCP 的 `isError: true` 会被映射成抛异常，走工具注册表的错误路径，让模型看到一个 `isError` 结果。这个映射很重要，否则模型会把错误信息当成正常返回。

还有一个安全细节：stdio 方式启动子进程时，桥接插件会清洗环境变量。所有名字看起来像凭证的环境变量，以及所有 `DSH_*` 开头的变量，都会被移除，只保留无害的环境变量。然后把你配置里 `env` 字段指定的变量加回去。这防止了 agent 的内部凭证（比如 API key）通过环境变量泄漏给第三方 MCP 服务器。

## mcp-memory：三个记忆服务器的接入手册

`examples/mcp-memory/` 目录不是内置功能，而是三份默认关闭的参考配置，演示如何用同一个桥接插件接入不同的第三方记忆系统。
...
注意一个时序细节：初始发现是异步的，发送第一个验证消息前要等 `mcp__` 开头的工具出现。如果 MCP 子进程崩了，桥接插件默认会按退避自动重连，断线期间工具注册保留、调用失败直到恢复；只有重连预算耗尽、或配置里显式关掉重连时，才需要重启或 HMR。

## 权衡

这套设计的代价和边界要讲清楚。只桥接了 tools：MCP 协议有三类能力（tools、resources、prompts），`dsh` 只接 tools，resources 和 prompts 在 harness 里没有消费者，暂时搁置，依赖这两类能力的场景当前用不了。启动超时继承自 MCP SDK：`dsh` 没有暴露连接/发现超时，用的是 SDK 默认的 60 秒，一个不响应的服务器可能拖慢激活和销毁。富媒体桥接只有图片：PNG、JPEG、WebP、GIF 可以在能力证明后进入原生上下文，audio 和内嵌 resource 留在执行局部，以诊断文本示人。记忆系统的检索质量取决于上游：比如 MCP Reference Memory 的搜索是大小写不敏感的子串匹配，不是语义检索，没有嵌入、没有自动摘要、没有冲突解决、没有遗忘策略，选哪个记忆系统本质上是选哪个检索能力，而不是选 `dsh` 怎么接。

回报是通用性。桥接插件是通用的，接入一个新的 MCP 服务器要写的只是一段配置，不用改一行 `dsh` 的代码。协议标准化的红利在这里兑现：N 个客户端和 M 个服务器，工作量是 N+M，而不是 N 乘以 M。

## 结论

`dsh` 对 MCP 的态度是把协议当生态接口，而不是当功能清单：一个通用桥接插件把任意服务器的工具变成 `ctx.tools` 上的原生工具，命名与 Claude Code、Codex 对齐，注册走两阶段代际切换，重连有预算，解析有防御。接入方只写配置，`dsh` 不替服务器做安装、认证、存储的任何决定。mcp-memory 示例用三份配置证明了这一点：换一个记忆系统，换的只是上游，桥接一行不改。
