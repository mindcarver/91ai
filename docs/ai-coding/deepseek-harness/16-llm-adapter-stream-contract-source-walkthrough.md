...
> 如果只能从这篇带走一句话：`dsh` 把"模型怎么调"压成了一条**封闭的流式协议**（`StreamChunk`），所有 provider 只负责往这条协议上吐 chunk，拼装、出错归一、重放、重试全由 harness 统一兜底。
...
- **adapter registry**：`registerAdapter(providers, adapter)` 把一个适配器注册到若干 provider 路由上。重复注册同一个 provider 会抛 `DUPLICATE_ADAPTER`，而且是 all-or-nothing，要么全注册成功要么一个都不动。注册返回一个 handle，既是 disposer 又能原子地 `replace(providers)` 换路由。
- **streaming call API**：`stream(options: GenerateOptions)` 发起一次调用，返回 `AsyncIterable<StreamChunk>`。

谁在用这个接缝？agent loop 在每个 step 组装好 `GenerateOptions`，调 `ctx.llm.stream()`；消费者（UI、压缩器、telemetry）要么直接读 chunk 流，要么用 `BlockAssembler` 拼装。三个官方 provider 全挂在同一个 `ctx.llm` 上：`llm-deepseek`、`llm-pi-ai`、`llm-replay`（测试用的录制回放适配器，从 fixture 文件里吐录制好的响应）。

关键的一点：`GenerateOptions.provider` 这个字符串选中适配器，`GenerateOptions.model` 传给那个适配器。模型 id 不需要在生命周期开始时注册，catalog 是 advisory（建议性的），适配器可以接受没列在 catalog 里的 model id。换句话说，路由认 provider，不认 model。
...
**`index` 而不是嵌套结构。** 一个 provider 同时流式吐两个工具调用的参数时，用 `index: 0` 和 `index: 1` 区分，而不是搞嵌套流。这把"交织"这件事降维成"给每个 delta 贴个下标"。

**`block-end` 带完整块。** 消费者不需要自己把 delta 拼回完整块，适配器在 `block-end` 时已经拼好了。这是把拼装责任放在适配器（它最懂自己的协议）还是放在公共 assembler 之间的一个取舍，`dsh` 选了"两边都给"：适配器吐 `block-end` 带完整块，同时 `BlockAssembler` 也能容忍只有 delta 没有 start/end 的协议。

**`finish` 是终态，带 `replayState`。** 一次成功的响应可以在 `finish` 里塞一段 adapter 私有的无损 JSON 状态，用来日后重建这个 provider 的原生响应。这个机制后面单独讲。

## 八条不变量：适配器必须守，消费者可以靠

契约文档在 `StreamChunk` 的 JSDoc 里列了一长串"every adapter MUST obey"。我把它浓缩成八条，每条都对应一个真实工程问题：

**1. `usage` 在 `finish` 之前，`finish` 之后什么都不许有。** 两个都推迟到 provider 的流结束标记，这样就不会出现一个只有 usage 的尾巴 chunk 破坏顺序。听起来琐碎，但这是让"统计 token"和"判断结束"能可靠并发的根基。

**2. 工具调用的 `arguments` 全程是原始 JSON 字符串。** 部分片段通过 `argumentsDelta` 流式传，那些原生返回解析后对象的 provider，在 `block-end` 时自己重新 stringify。为什么不让适配器吐对象？因为流式场景下对象没法增量拼，字符串可以。
...
这八条不是文档摆设。每一条都能在源码里找到对应的断言或测试。违反任何一条，要么编译期炸，要么运行期在不变量检查里炸，要么在 wire 测试里挂。
...
`dsh` 的做法是在适配器边界就把它们翻译成统一的 `code`。两个 DeepSeek 适配器都实现了 `isContextWindowExceededError()`，把"上下文超长"这件事无论以何种形式到达，都映射成 `CONTEXT_WINDOW_EXCEEDED`。空响应映射成 `EMPTY_RESPONSE`。看门狗超时映射成 `TIMEOUT`。调用方主动 abort 保持 `ABORTED`。
...
**第一个是 per-message 的 `replayState`。** 一次成功的 `finish` 可以带一段 adapter 私有的无损 JSON 状态，agent loop 把它和拼好的 assistant 消息一起存进会话日志。日后如果要把这个 provider 的原生响应重建出来（比如跨模型迁移、跨 provider 转换），这段状态就是原料。`LlmRuntime` 的规则是：只有当历史消息的 provider 和当前目标 provider 当前都注册在**同一个适配器实例**上时，才把这段状态传给那个适配器。适配器自己校验状态、自己负责跨模型或跨 provider 的转换。别的适配器只会收到 provider 中立的 content 加上 provider/model 字段，拿不到这段私有状态。
...
两件回事：一个是消息级别的私有重建状态，一个是测试级别的整 provider 替身。读到源码里 `replayState` 字段和 `llm-replay` 包名时，要知道它们解决的不是同一个问题。
...
光讲契约容易飘。给一个具体落点。`packages/llm/llm-deepseek/src/` 下的 DeepSeek 适配器是 direct-fetch 实现（不依赖第三方 SDK），它的职责就是：

- `translate.ts`：把 provider 中立的 `GenerateOptions` 翻译成 DeepSeek 的 HTTP 请求体。
- `sse.ts`：解析 provider 返回的 SSE 流。
- `adapter.ts`：实现 `stream()`，把 SSE 事件翻译成 `StreamChunk` 序列 yield 出去，包括 `isContextWindowExceededError()` 这类归一化逻辑。
...
**封闭联合的扩展成本高。** 加一个 chunk 变体，所有消费者编译失败。好处是类型安全，坏处是想加一个 provider 专属的、别的 provider 用不上的流式事件，会很别扭。`dsh` 的态度是：新模态只有当它的适配器、UI、压缩、持久化重放路径都支持了，才允许进 `ContentBlockMap`。

**适配器契约严格，学习曲线陡。** 八条不变量、replayState 的所有权规则、归一化的 code 体系，写一个合规适配器不是半小时的事。官方专门有一篇 cookbook 带你写一个（接 OpenAI 兼容模型），那个流程本身也说明门槛真实存在。

**调试链长。** 一个请求从 `agent/request` 到 `llm/stream` waterfall 到适配器再到 provider HTTP，中间好几层。出问题时定位"是适配器翻译错了还是 provider 返回怪"需要对着 chunk 日志看。这是全插件化架构的共性代价。

回报是：接一个新模型不用碰 agent loop，错误处理统一，token 计费口径一致，测试能完全离线确定性回放。对一个要支撑"任意 OpenAI 兼容端点"的 harness 来说，这个取舍是划算的。

## 结论

`ctx.llm` 把"调模型"这件事压成了三个东西：一个注册表（适配器挂 provider 路由）、一个封闭流式协议（`StreamChunk` 七变体）、一套把 provider 花样收拢成统一 code 的归一化层。适配器只实现 `stream()` 吐 chunk，拼装交给唯一的 `BlockAssembler`，错误交给 `LlmFailure`，重试交给策略层，扩展交给 `llm/stream` waterfall。
