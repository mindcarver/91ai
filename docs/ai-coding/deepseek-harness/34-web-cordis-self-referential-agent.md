...
**`cordis_run`**：真正让包活起来。host 半边在 vm 沙箱里求值，在 `cordis-dynamic` group fiber 下运行。如果包有 client 半边，这一步变成一个需要人确认的往返：发出 `cordis/request-run` 事件，挂起，等一个打开的浏览器页面允许或拒绝。第一个回答赢。
...
这意味着动态包是**实验性的、临时的**。如果你想把一个实验固化下来，README 的指引是：让 agent 把它实现成一个正常的本地、项目或仓库插件，走常规的开发流程。动态包不是插件的替代品，是插件的草稿纸。
...
**成功运行不等于 UI 渲染成功。** `run` 在页面加载了 client 半边后就返回了，React 渲染在之后发生。一个组件抛异常不会出现在 run 的回执里。渲染失败通过 `reportRenderFailure` 异步报告，用 `cordis_inspect what:"temporary"` 读回来。run 结果会说明这一点，而不是暗示成功。

**无浏览器的部署会挂起。** 一个有 client 半边的包在 headless 或 ACP 部署里，run 会一直挂起，直到发起的 turn 被取消。因为转发的事件不报告谁收到了它，没人知道有没有页面会回答。host-only 的包不受影响。

**挂起的 run 请求没有超时。** 它等人回答，唯一的退出方式是发起 turn 的取消。无人值守的自动化用不了有 client 半边的包。

**vm 超时只约束同步部分。** `vmTimeoutMs`（默认 5000ms）只管 host 半边的同步执行。一个 async 的 host 半边体逃逸这个限制，和工具集的协作式信任立场一致。
