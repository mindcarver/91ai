...
理解 `dsh` Web 客户端的第一件事：它不是传统的"后端渲染前端"。它是两半独立的插件图，各自有完整的生命周期。

**Host 半**（Node.js 进程）：所有 agent 能力、模型适配器、工具、会话、沙箱、审批。这是 `dsh` 的主体，跑在服务器或本地进程里。

**Client 半**（浏览器）：UI 组件、Conversation Node 渲染器、交互逻辑。这是一组独立的 Cordis 插件，在浏览器里加载和激活。
...
`ctx.clientModules`（`ClientModuleRegistry`）是连接两半的关键服务。它做四件事：
...
扫描是**按包增量**的，没有全量重扫代码路径。每次 cordis 的 `internal/plugin` 事件（fiber 构造或销毁）标记那个 fiber 的条目名为 dirty，一个 microtask flush 把每个 dirty 名字和 live loader 条目对账。
...
每行的 `rev` 是 bundle 的内容哈希，搭在 URL 上做 cache-busting（`/plugins/<id>/client.js?rev=<rev>`）。图的 `rev` 哈希所有组合的行，任何一行变化都改变它。`immediately` 标记的行在 module-face boot 期间预取和执行（只注册 factory）；lazy 行在首次 import 时获取。
...
`rebuilt(id)` 是 bundle 内容到达图的唯一入口。它重新哈希文件，只有真正的 rev 变化才重组图并通知。`onRebuilt` 在每个变化的 bundle 时触发，带新 rev。`onGraphChanged` 在任何重组图的 flush 后触发，是 pull 模型：监听器重新读 `graph()`。

生产图完全省略 HMR 行。module host 自己从不 watch 文件。

这套 HMR 机制让开发时改一个 client 插件的代码，浏览器自动拿到新 bundle，不用手动刷新。背后的实现是 stat-poll（不是 fs.watch，避免跨平台不一致）加 SSE 推送。
...
一个 client 包要贡献 Chat Node，需要：
1. 在 `package.json` 声明 `dsh.client`（让它进入 Client Modules 扫描）
2. 在 `exports["./client"]` 导出构建后的 bundle
3. 在 bundle 的 `apply` 里注册 Definition 和 renderer

之后，Client Modules 把它组装进 boot graph，浏览器加载它，它注册自己的 Node 和 renderer。整个过程不需要改 Host 的任何代码。
...
ACP server（33 篇拆过）是另一种多 agent 协议入口。它让外部程序通过 JSON-RPC 创建和驱动 agent。Web 客户端和 ACP server 可以共存：Web 给人用，ACP 给程序用，两者共享同一个 Host 进程和 agent 组合。

## 权衡与局限

**两半分离增加复杂度。** 你要同时理解 Host 和 Client 的插件图、wire 协议、RPC 机制。一个完整的 client 贡献涉及跨进程的类型同步和构建顺序依赖。

**wire 协议是 boot 级别的。** `window.__DSH_BOOT__` 在页面加载时注入一次。如果 Host 组合在页面加载后变了（HMR 加了新插件），需要重新加载页面才能拿到新 manifest。HMR 的 SSE 机制处理 bundle 内容变化，但不处理 manifest 结构变化。
...
**Stat-poll 不是实时的。** HMR 用 stat-poll（默认 500ms 间隔），不是 fs.watch。bundle 变更的检测有几百毫秒延迟。这是跨平台一致性的代价。
