...
<p align="center">
  <img src="../../assets/agent-learning-cover.png" alt="Agent Learning Path cover" width="100%" />
</p>
...
### Stage 3-4: Workflow 与 Agent Loop
...
### Stage 5-6: Memory 与 Agentic RAG
...
### Stage 9-10: Productionization 与 Multi-Agent
...
| 2 | Tool Calling | 能让模型安全调用外部函数和 API |
| 3 | Workflow | 能把多步骤任务拆成可控流程 |
| 4 | Agent Loop | 能设计 observe -> plan -> act -> reflect 的执行循环 |
| 5 | Memory | 能区分短期上下文、长期记忆和项目知识 |
| 6 | RAG for Agent | 能让 Agent 使用外部知识而不是凭空回答 |
| 7 | Evaluation | 能评测任务完成率、工具调用质量和失败模式 |
| 8 | Safety & Permissions | 能控制工具权限、数据边界和高风险操作 |
| 9 | Productionization | 能上线、观测、回放、降级和持续改进 Agent |
| 10 | Multi-Agent | 能判断什么时候需要多个 Agent，什么时候不需要 |
